...
- Border radius is hierarchical: `--radius-xs` (4px) for badges, `--radius-md` (12px) for inputs and standard containers, `--radius-xl` (24px) for Card/EntityCard navigation tiles, chat bubbles, and the chat card furniture (MessageCard, ToolCall, AgentPlan, InterruptCard), `--radius-full` (999px) for buttons.
...
**`ds-gauge`** — A radial dial for a single bounded reading: capacity, usage, a score against a target. Like Sparkline, dependency-free SVG with no `'use client'`, so it renders from a Server Component and keeps its own folder rather than joining `Chart/`. The dial is a three-quarter circle (270° sweep, gap at the bottom) drawn as two `pathLength=100` circles: a `--color-bg-container-tertiary` track and a value arc in `currentColor`, animating between readings via a `stroke-dasharray` transition at `--motion-duration-slow` `--motion-ease-emphasized`. Colour flows through `color` per tone — `accent` (default) `--color-action-primary-bg`, then the `positive`/`warning`/`error` status **border** tokens (stable across themes, where the dark-mode text tints are near-white and would wash the tones into one) and `neutral` `--color-text-secondary` — and `thresholds` recolour the dial as the reading crosses them (highest threshold at or below the value wins), which is how a capacity dial goes amber at 70 and red at 90. The centre stacks the reading (`--font-heading-3-*`, `--color-text-primary` so its contrast never depends on the tone, formatted via `formatValue`) over an optional caption (`--font-caption-*`, `--color-text-secondary`) that doubles as the accessible name. `size` and `strokeWidth` are SVG geometry in pixels, not tokens. Announced as `role="meter"` with `aria-valuenow/min/max` and the formatted reading as `aria-valuetext`; a degenerate range (`max <= min`) draws an empty dial rather than NaN.
