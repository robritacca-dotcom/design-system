...
**`ds-funnel-chart`** — Ordered stages as side-by-side vertical bars whose heights step down with each stage's share of the first, each carrying a centred percentage pill. Dependency-free JSX/CSS like Sparkline: no recharts, no `'use client'`, renderable from a Server Component. It wears the chart family's shared chrome — the `ds-chart` card, header (`title`/`subtitle`), and padding come from `Chart.css`, with `bare` stripping them inside a Panel. Stage colours cycle `--color-chart-series-1` → `-7`; bars are `--radius-sm` with per-stage height and colour flowing in as inline custom properties so the stylesheet stays token-only. Shares clamp to a floor (`minStageShare`, default 16%) so steep drop-offs stay readable, while the pill prints the true percentage. Each stage's hover target is its full-height column, like the recharts tooltip cursor: hovering anywhere in the column tints the band (`--color-bg-container-secondary` at 0.4, the same recipe as the recharts cursor) and reveals the family's glass tooltip — stage name over its reading (`displayValue`, falling back to the locale-formatted value) — anchored above the percentage pill and revealed by CSS alone, with the end stages pinning to their own edge so the overlay stays inside the card; the values are already in the row's accessible label, so the overlay is decorative. The stage row is `role="img"` with a generated label naming every stage and value. Pair with a LegendTile row for the stage readings.
