...
| streaming | `boolean` | no | `false` | A response is streaming: the send button becomes a stop button, submit is blocked, and Enter is inert. |
...
| aiGlow | `boolean` | no | `false` | While focused, the shell wears AiButton's slowly rotating gradient ring and glow in place of the plain selected border — the system's "a model answers here" signal, for composers whose messages are answered by one. Off by default. |
