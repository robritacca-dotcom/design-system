...
| MCP tool descriptions, server instructions, and the browser landing page (`website/src/app/api/mcp/route.ts`) | None ("you" for the landing page's instructions) | One or two lines per tool; one short page for a person who typed the URL | Read by agents choosing a tool and by people checking what the URL is: state what it returns, never promote | This file |
| Site-chat widget copy (the welcome tagline and disclaimer line in `website/src/components/SiteChat/SiteChat.tsx`) | Imperative | One line each | The tagline names subjects, not the author; the disclaimer links to /privacy for the AI-use and logging disclosure and fits one line at the caption size | This file + the `content-audit` skill's `chat` scope |
