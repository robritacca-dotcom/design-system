...
**A newly synced essay does not build on its own — two gates.** Every essay needs its illustrated cover pair (an entry in `website/src/data/essay-covers.json` plus the light and dark files under `website/public/covers/writing/`), and `scripts/validate-essay-covers.mjs` fails the website build until both exist; it also needs a hand-written TLDR entry under the `essays` map in `website/src/data/page-summaries.json`, or `scripts/validate-page-summaries.mjs` fails the build the same way. The sync script's doc block names both. This loop *can and should* write the summary entry itself — a title, one TLDR sentence, and 1–2 chips per the validator's field grammar and content-design.md's Page summaries row — in the same commit as the sync. It cannot draw a cover, so when the sync brings in an essay with no pair: leave the synced `essays.json` change (and the summary entry) out of the loop's commit (`git checkout` them back in the worktree), finish the rest of the loop, and say in the report that a new essay is waiting on its cover before it can be synced. Never try to satisfy either validator with a placeholder.
