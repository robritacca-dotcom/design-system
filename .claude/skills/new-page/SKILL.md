...
- **Deliberately hidden pages are the exception to both rules**: a test bench or scratch page that must stay dark sets `robots: { index: false, follow: false }`, skips the canonical *and* the sitemap entirely, and records why in a comment in its `layout.tsx` — `website/src/app/robr0-gpt/layout.tsx` is the precedent
