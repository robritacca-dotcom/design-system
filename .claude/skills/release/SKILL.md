...
- If the publish step fails with a **401**, the `NPM_TOKEN` secret has expired — it needs regenerating on npmjs.com and re-adding as a repo secret. Rob does that; never handle the token value. This is also the moment to consider migrating to Trusted Publishing (OIDC), which retires the token entirely — the workflow already grants `id-token: write`.
