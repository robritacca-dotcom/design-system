...
2. **Decide where it runs.** Two slots exist, and the source of the data decides:
   ...
   - **The post-website-build slot** in `verify` (root `package.json`) and CI's website job (`.github/workflows/ci.yml`): for checks that need the prerendered HTML the website build writes. `validate-rendered-spacing.mjs` and `validate-corpus-coverage.mjs` are the two residents; each one's doc block argues why source-level checking would over- or under-report, and a new arrival owes the same argument.
