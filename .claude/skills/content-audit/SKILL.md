...
1. **Determine scope.** Accept one of:
   ...
   - `palette` → the command palette's shipped copy: the group labels, per-item descriptions and the search placeholder in `website/src/components/SitePalette/` (reachable from every non-chromeless page, but outside `app/**`, so the `website` scope misses it — the same gap the `chat` and `footer` scopes close for theirs; no validator scans it). The ask-chat row's label is the visitor's own typed query — unauthored by design, never a finding; the empty state is unreachable (the ask row matches every query), so there is none to audit
