...
1. **Determine scope.** Accept one of:
   ...
   - `chat` → the site chat's shipped prose: the greeting and starter-suggestion copy in `website/src/components/SiteChat/`, the persona and easter-egg strings in `website/src/app/api/chat/`, and the playground sim's scripted story — the turns and chip labels in `website/src/lib/chat-sim.ts` and the event-rail copy in `website/src/app/playground/ChatDirector.tsx` (all visitor-visible but not "page files", so the `website` scope misses them; `scripts/validate-shipped-prose.mjs` defers non-page `.ts` modules to this scope by design, and the chips' register row lives in the guide's Register by Surface table)
