...
Deliberately minimal: no logo, no brand row. A `FadeDivider` on top — the shared 1px hairline that fades to `--color-bg-container-primary-transparent` at both ends, the same line the home hero and the DS landing draw — then five link columns under a `<nav aria-label="Footer">` (Site, Design system, Docs, Work, Elsewhere), then a quiet bottom row. Columns derive from the nav config (`dsMegaItems`, `docsSidebarLinks`, `workSidebarLinks`) and `website/src/config/social.ts`; nothing is hand-listed twice. Column titles sit in `--font-title-body-weight` at `--font-paragraph-sm-size` on `--color-text-primary`; links are `--font-paragraph-sm-*` on `--color-text-tertiary`, hovering to `--color-text-primary` on `--motion-duration-base`. External links carry a trailing `open_in_new` at `--icon-size-sm`. The bottom row holds the monochrome social icon links (36px targets, `currentColor` at `--color-text-tertiary`) and the copyright, left-clustered so the fixed chat button never covers a link; on phone widths it adds bottom clearance for the same reason. The grid steps 5 → 3 → 2 columns at the site breakpoints. As a body flex child it inherits the docked-chat inset and pins to the viewport bottom on short pages via `margin-top: auto`. Focus rings are the sanctioned `--color-action-primary-bg` outline; teal appears nowhere else in the footer.
