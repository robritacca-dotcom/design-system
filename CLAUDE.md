...
- **References are build-checked**: `scripts/validate-doc-refs.mjs` fails the build when a skill or doc (this file, `README.md`, `design.md`, `content-design.md`, `SECURITY.md`) references a repo path, `npm run` script, or documented API symbol that doesn't exist.
...
└── website/                   # Next.js docs site (npm workspace; consumes @robr0/design-system by name)
    ...
    ├── src/app/
    │   ├── components/        # One folder per component (page.tsx + page.module.css) plus one per category (a three-line shell over ComponentCategoryPage); the index page renders registry-derived category sections over the shared ComponentPreviews map
    ...
    └── src/components/        # Shared Next.js UI (MegaNav header, Sidebar, SiteFooter + SiteChat — both mounted once from the root layout, skipping the chromeless routes in src/config/chromeless.ts; BlurBackground composes the ambient background and is mounted once from the layout too — the site's fixed layer, the CSS blob fallback, the config and the dev tuner, wrapped around the library's ShaderField, which owns the WebGL2 renderer itself)
```
...
6. **Register it** — one entry, and most surfaces follow automatically:
   ...
   - `website/src/components/ComponentPreviews/ComponentPreviews.tsx` — add a preview entry under the component's slug. This is the one surface still hand-maintained, because each preview is a bespoke miniature; the index section, the category landing page, and the sidebar group all derive from the registry.
7. **Document it in `design.md`**: add a short component spec section (class name, tokens used, key behaviours).

Steps 5–7 are build-enforced by two validators: `scripts/validate-website-surfaces.mjs` fails the build if any public component is missing its showcase page, preview entry, or `design.md` spec (the nav entry and its order are derived from the registry, so they cannot drift); `scripts/validate-page-titles.mjs` fails it if the page has no `layout.tsx`, or if that layout does not derive its title via `componentPageMetadata("<slug>")` — and it holds the category landings to `categoryPageMetadata("<id>")` the same way.
...
- [ ] Preview entry added to `ComponentPreviews.tsx` under the component's slug (build-enforced). The index section, category landing page, sidebar nav, sitemap and breadcrumbs derive from the registry — nothing to add
