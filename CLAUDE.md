...
| Component website surfaces | `src/components/registry.json` (same registry) | — | `scripts/validate-website-surfaces.mjs` — every public component has a showcase page, index-grid `TocCard`, and a `###` spec section in `design.md`; also keeps `SECTION_OG_IMAGE_SEGMENTS` in `website/src/config/navigation.ts` in sync (both directions) with the section-level `opengraph-image.tsx` files, so sub-page share cards can't silently fall back to the root card. It also holds `/foundations/colour-mode` to the colour token registry in **both directions**, so a colour token can never exist without a swatch on the page that states it documents all of them. The sidebar nav entry and its alphabetical order are no longer checked because `componentsSidebarLinks` is **derived** from the registry — they cannot drift |
...
| Cover renders | `website/src/data/cover-renders.json` (which aspects each study's cover is shot at, plus each aspect's ratio and pixel width) | — (nothing displays a count; `COVER_RENDER_TARGETS` + `coverRenderSrc` from `website/src/data/cover-renders.ts` are the accessors) | `scripts/validate-cover-rasters.mjs` — every registered render has a file under `website/public/covers/rendered`, every file there is registered, and the study list matches `CASE_STUDY_COVERS` in both directions. **Staleness is deliberately not checked** — telling a stale shot from a fresh one means re-running a browser on every build, so regenerating after a cover change (`npm run covers:render`) is the author's job, like the essay sync |
...
| Ambient background | `website/src/data/shader-background.json` — the renderer switch, the seven shader parameters, and the eight blob definitions. Hand-edited: tune a look live with the dev-only panel (`?tune=1` on any page in dev), then paste the panel's snippet back in. Setting `"mode"` to `"css"` reverts the whole site to the CSS blobs, which are always rendered underneath as the fallback | — (nothing displays a count; `shaderBackground` from `website/src/data/shader-background.ts` is the accessor) | `scripts/validate-shader-background.mjs` — mode is a known renderer, every parameter is inside the tuner's slider range, the blob count matches `BLOB_COUNT` in the shader source, and **every blob's colour token exists in the token registry**, so a renamed token cannot leave the background sampling a property nothing defines. `scripts/validate-single-background-mount.mjs` — `BlurBackground` is mounted exactly once, in the root layout: mounted per page it would rebuild the WebGL context on every navigation, and a second mount would stack a second canvas and context on the first |
