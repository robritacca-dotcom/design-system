...
- **References are build-checked**: `scripts/validate-doc-refs.mjs` fails the build when a skill or doc references a repo path, `npm run` script, or documented API symbol that doesn't exist (the `sources` list in that script is the authoritative set of docs — the root specs, SECURITY.md, and the workspace, eval and ga-analysis READMEs).
...
└── website/                   # Next.js docs site (npm workspace; consumes @robr0/design-system by name)
    ...
    ├── src/components/covers/ # EssayCover (the essays' illustrated light/dark pairs, see the essay-covers registry) and the vector redraws of the screens each case study is known by, drawn in CSS/SVG rather than captured (the mega panel's showcase card draws PipelineWireframe instead — a token-drawn abstract of the pipeline in website/src/components/PipelineWireframe/, live vector, no raster): one 16:10 CoverFrame per cover, mapped to pages by case-study-covers.tsx, staged on /covers. The redraws are deliberately token-free — every value is a drawing coordinate from the source frame, sanctioned per module by a ds-allow-file(mockup) header, so a theme change can never alter a picture of what shipped. The drawing renders in two places only — the /covers grid and the /covers/render shot surface; everywhere a cover is *displayed* reads a flat image via CoverImage, because mobile WebKit does not render an HTML mock inside a scaled SVG foreignObject reliably
