...
| Site chat corpus | `website/src/data/site-corpus.generated.ts` — **generated** from the published site (navigation, page prose via the TypeScript AST, the data registries above, and the three root specs) by `scripts/generate-site-corpus.mjs`, never hand-edited | `siteCorpus` + `siteCorpusApproxTokens` from the same file | `scripts/validate-site-corpus.mjs` — regenerates in memory and byte-compares, checks for leaked details (local paths, analytics ids, keys, email addresses), and re-checks the token budget |
...
**The site chat corpus is public-only, and that is a security boundary, not a style choice.** `scripts/generate-site-corpus.mjs` may only read sources that are already published — page prose, the data registries, the blueprint specs. The corpus becomes the chat model's context, so anything in it can be repeated verbatim to any visitor who asks. Keeping it public-only means the worst case of a successful prompt injection is off-brand prose rather than a leak. Adding a private source would quietly convert one into the other, so don't; `scripts/validate-site-corpus.mjs` screens for the obvious slips (local paths, analytics ids, keys, email addresses) but it cannot judge whether a new source was meant to be public.
...
└── website/                   # Next.js docs site (npm workspace; consumes @robr0/design-system by name)
    ...
    ├── src/lib/               # Non-UI modules (chat-sim + chat-transport, Substack feed, OG image, structured data)
