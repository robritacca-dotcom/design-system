...
| Command palette copy (group labels, item descriptions and the placeholder in `website/src/components/SitePalette/`) | None | Group labels and one-line fragments; the placeholder as microcopy | Navigation rows' descriptions name destinations; action rows (the Actions group, the ask-chat row) name what selecting them does. The ask row's own label is the visitor's typed query — deliberately unauthored, never restyled. The empty state is unreachable by construction (the ask row matches every query), so the palette ships none | This file + the `content-audit` skill's `palette` scope |
